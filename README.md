...
- Hlavní uživatelské UI skládá nad uloženými `CatalogTitle` read-only season view-model. Supplementary část se v něm sbalí pod sezónu jen při právě jednom přesném matchi `effective_season_number`; bez kontextu, bez odpovídající sezóny nebo při nejednoznačnosti zůstává dostupná jako anime-level **Další část**. Jediná hlavní sezóna s pouze navázanými doplňky se stále otevře přímo; Hierarchy Review nadále ukazuje všechny skutečné `CatalogTitle` samostatně.
