...
- Každý současný `CatalogTitle` lze přímo v Hierarchy Review autoritativně klasifikovat jako Season, Part, Film, OVA, Special, Preview, Recap, Bonus nebo Other. Pro Part se zvlášť ukládá season scope a pořadí Partu. Legacy `cour` zůstává čitelný kvůli kompatibilitě, ale není nabízen jako nová hlavní uživatelská jednotka. Strukturální typ části je oddělený od video-level `content_type_manual`; Film lze výslovně zvolit také pro konkrétní video.
- `Video.file_type` zůstává uloženým výsledkem filename parseru. Effective klasifikace konkrétního videa používá prioritu explicitní `Video.content_type_manual` → konkrétní filename/raw typ → supplementary kontext `CatalogTitle`; potvrzený nebo bezpečně odvozený Film se proto zobrazuje jako Film i při parserovém `other`, ale ruční video-level klasifikaci nepřepisuje. Samotné slovo `Movie` ve filename film neurčuje.
...
- Media Check, jeho filtry, title detail i CZ/SK/Bez CZ/SK souhrny hlavního katalogu používají stejný effective compatibility resolver. Completion je per fyzická video reprezentace/varianta; BD↔TV, A↔B ani known↔`NULL` se automaticky nesdílejí. Pouze secondary s aktuálně `VALID` duplicate relation nevytváří novou povinnou completion jednotku; `INVALID` a `UNKNOWN` evidence zůstává aktivní. Internal subtitle, hardsub a nullable manual-unavailable workflow se významově nemění; skutečná pozitivní CZ/SK evidence má před starším unavailable markerem faktickou přednost, aniž marker automaticky maže.
...
- `duplicate_of_video_id` zůstává ruční evidencí potvrzené duplicity. Shared runtime resolver rozlišuje `VALID`, `INVALID` a `UNKNOWN`; pouze prokazatelně `VALID` secondary se kolabuje. `INVALID` i `UNKNOWN` zůstávají aktivní, evidence se automaticky nemaže a Hierarchy Review zobrazí konflikt, respektive neověřitelný stav. Historický confirmed vztah mezi dvěma různými non-`NULL` groups se automaticky nepřepisuje.
...
- Automaticky nalezená unresolved duplicita, ruční podezření a potvrzená evidence přes `duplicate_of_video_id` jsou tři nezávislé stavy. Unresolved kolize blokuje Hierarchy Review. Potvrzená relation je logicky vyřešená pouze ve stavu `VALID`; při `INVALID` nebo `UNKNOWN` se secondary nekolabuje a vyžaduje kontrolu, ale evidence zůstává uložená. Pokud primary chybí, `duplicate_primary_missing` je znovu blokující problém a AnimeDB náhradu automaticky nevybírá. Ruční označení `suspected` nevybírá primary, nic nemaže a nemění hierarchii, metadata ani typ obsahu.
...
- Typed local ordinal se posuzuje ve společném namespace authoritative structural contextu a effective video typu uvnitř `CatalogCollection`, včetně Bonus, Film, Menu a Other; Preview/PV namespace sdílejí. Supplementary titles stejné hlavní části sdílejí namespace, různé hlavní části nikoli; bez attachmentu se použije anime-level/root. Povinný je až při 2+ distinct logical identities, nikoli podle počtu Video rows. Singleton může zůstat bez čísla a jeho existující bezpečný ordinal se nemaže. Potvrzené duplicity, varianty známé identity a úplné Media Parts count nezvyšují. Episode musí mít logical číslo vždy; fractional Recap si zachovává přesnou chronologickou pozici bez dalšího ordinalu. Parserový Special05 se po ruční změně typu na Bonus automaticky nestává Bonus05; explicitní manual ordinal 5 ano. Missing identity při multiplicitě a nevysvětlené kolize se odvozují read-only do Hierarchy Review i pro stored `automatic`/`verified`. Podrobná současná pravidla jsou v [PROJECT_STATUS](docs/PROJECT_STATUS.md); původní [audit a inventura](docs/SUPPLEMENTARY_ORDINAL_AUDIT_2026-09-05.md) jsou historickým podkladem.
- Parser zachovává ordinal také u úzkých forem `NCOP Ver.TV1` (stejně OP/ED/NCED), `(PV 01)`, koncového `_bd_spec_02` a `NCED 02a/02b`. TV a A/B jsou pouze oddělené parserové hinty; shodné číslo samo nepotvrzuje variantu ani duplicitu. [Re-audit devíti kandidátů](docs/SUPPLEMENTARY_CANDIDATE_REAUDIT_2026-09-05.md).
...
- **Správa zařazení jednotlivých videí** a editor **Typ obsahu** v detailu používají stejné canonical video typy a existující klasifikační workflow. Volba **automaticky** clearne `content_type_manual`; raw parserová evidence zůstává zachovaná. Workflow **Oddělit do nové části** mění strukturální typ `CatalogTitle`, ale nevytváří ani nepřepisuje `Video.content_type_manual`; ten mění pouze výslovná akce klasifikace konkrétních videí.
