...
- Supplementary ordinal je lokální logická identita uvnitř `CatalogTitle` a typu, nikoli nutně providerové/canonical číslo. Pro OP/ED/NCOP/NCED/OVA/Special/Preview/PV/CM platí ruční video autorita → bezpečný filename ordinal → unknown; běžné `Episode 14` v OVA části samo není OVA14. Media Parts a potvrzené varianty jsou oddělené osy, ne další ordinaly. Ruční editor ukazuje effective prefix odděleně od numerického integer vstupu a výslednou identitu ve tvaru `NCOP 01`; složené hodnoty ani A/B nebo variantní značky nepřijímá. Chybějící identita v opakovatelném typu, nevysvětlená ordinal kolize a porušená duplicate identita se odvozují read-only do Hierarchy Review i pro collection se stavem `automatic` nebo `verified`; uložený `hierarchy_status` se tím nemění. Budoucí V6 rename musí použít bezpečnou identitu nebo vyžádat review a nesmí vytvářet kolidující cesty. [Audit a inventura](docs/SUPPLEMENTARY_ORDINAL_AUDIT_2026-09-05.md).
