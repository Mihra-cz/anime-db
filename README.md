...
- Každý současný `CatalogTitle` lze přímo v Hierarchy Review autoritativně klasifikovat jako Season, Part, Cour, Film, OVA, Special, Preview, Recap, Bonus, Other nebo Title. Stejný potvrzený POST workflow a stejný centrální seznam typů používá detail collection; `Film` je part-level typ a nepřidává se do video-level `content_type_manual`.
...
- **Správa zařazení jednotlivých videí** zachovává samostatnou video-level klasifikaci Recap, Preview, Special, OVA, Bonus a Other. Workflow **Oddělit do nové části** používá plný part-level seznam; při typu Film/Season/Part/Cour/Title nevytváří nepodporovanou video-level hodnotu.
